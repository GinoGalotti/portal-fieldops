...
| `missions/journal.html` | `keeper.css` | Keeper | Field Journals — data-driven from `data/journals.json`. Renders personal archive notes per author/narrator. Block types: `prose`, `editorial`, `scene`, `questionnaire`, `pull-quote`, `thread`. Each block has `data-block-id` for future D1 comments. Multi-author tab switcher (shows if >1 journal). Per-operative accent colors via CSS custom properties (`--op-accent`). Currently: John Johnson Sessions 01–02. |
...
| `portal-db-custom.json` | PORTAL-authored custom entries for `missions/entities.html` Section III. Currently: Shōjō. Semantic schema (v2): `id`, `display_name` (HTML string), `custom: true`, `type`, `harm`, `armour`, `extra_tags[]` (`{label, color}`), `motivation`, `type_detail`, `harm_color`, `harm_note`, `armour_detail`, `origin`, `attacks[]` (`{name, value, full}`), `weakness`, `custom_moves[]` (`{name, text}`), `notes[]` (`{label, text}`). **Add a new custom entry:** append to `entries[]` — no HTML edit needed. | `missions/entities.html` Section III |
| `portal-db-deck.json` | 53 Deck of Monsters archive entries for `missions/entities.html` Section III. Semantic schema (v2): `id`, `name`, `type`, `harm`, `armour`, `motivation`, `powers`, `attacks[]` (`{name, value}`), `weakness`, `custom_moves[]` (`{name, text}`), `description`. **Add a new deck entry:** append to `entries[]` — no HTML edit needed. | `missions/entities.html` Section III |
...
| `journals.json` | **Field Journals** — personal in-character notes from non-fielded operatives. Schema: `version`, `journals[]`. Each journal: `id`, `author`, `role`, `tagline`, `accent` (hex), `entries[]`. Entry types: `preamble`, `case-file` (case_number + session + title + subtitle), `threads`, `closing`. Block types: `prose` (text[]), `editorial` (text string), `scene` (heading + optional subheading + text[]), `questionnaire` (heading + subheading + question + responses[]), `pull-quote` (speaker + text), `thread` (number + heading + text[]). Each block has a stable `id` for future D1 comments feature. Currently: John Johnson Sessions 01–02. | `missions/journal.html` |
